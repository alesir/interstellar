...
![screen](https://github.com/alesir/interstellar/blob/master/Interstellar_crash.png)
...
_Interstellar_ downloads crashes via google-provided [gsutil](https://cloud.google.com/storage/docs/gsutil) tool and triggers [Slack incoming webhook](https://api.slack.com/incoming-webhooks) for all new or updated crashes.
...
1. create a file `secrets/secrets.yml`. There is an example in `secrets/secrets.yml.example`.
  ...
  - report bucket id. Found in the Crashes & ANRs page of Google Play Developer console, e.g. `pubsite_prod_rev_01234567890987654321`
...
`system “BOTO_PATH=./secrets/.boto gsutil/gsutil cp -r gs://#{CONFIG[“app_repo”]}/crashes/#{csv_file_name} .”`
